...
def compute_champ_freq(df):
    all_champs = pd.concat([df[[f'blue{i}' for i in range(1,6)]].stack(),
                             df[[f'red{i}' for i in range(1,6)]].stack()])
    return all_champs.value_counts().to_dict()

def treinar_word2vec(df):
    sentences = [[row[f'blue{i}'] for i in range(1,6)] + [row[f'red{i}'] for i in range(1,6)] for _, row in df.iterrows()]
    model = Word2Vec(sentences=sentences, vector_size=64, window=10, min_count=5, workers=4, sg=1, epochs=100, seed=77)
    return model
...
def gerar_features(df, model_w2v, numeric_cols, league_encoding=None, champ_freq=None):
    ...
    for idx, row in df.iterrows():
        ...
        full_vector = np.concatenate([emb_blue, emb_red, extras, league_feats])
        X.append(full_vector)
    return np.vstack(X)

def apply_dynamic_threshold(y_true, y_proba):
    return max(((thr, f1_score(y_true, (y_proba >= thr).astype(int))) for thr in np.arange(0.01, 0.99, 0.01)), key=lambda x: x[1])[0]
...
def treinar_pipeline(df, checkpoint, league_encoding_global, champ_freq=None, forcar_retreinamento=False):
    model_path = f"models/model_{checkpoint}.joblib"
    meta_path  = f"models/meta_{checkpoint}.joblib"
    ...
    X_train_df, X_val_df, y_train, y_val = train_test_split(df_cp, y, test_size=0.2, stratify=y, random_state=77)

    champ_freq = compute_champ_freq(X_train_df) if champ_freq is None else champ_freq
    model_w2v = treinar_word2vec(X_train_df)

    league_encoding = league_encoding_global if checkpoint == 'draft' else None

    X_train = gerar_features(X_train_df, model_w2v, numeric_cols, league_encoding, champ_freq)
    X_val   = gerar_features(X_val_df, model_w2v, numeric_cols, league_encoding, champ_freq)

    scaler = StandardScaler()
    if numeric_cols:
        start = 2 * model_w2v.vector_size
        X_train[:, start:start+len(numeric_cols)] = scaler.fit_transform(X_train[:, start:start+len(numeric_cols)])
        X_val[:, start:start+len(numeric_cols)] = scaler.transform(X_val[:, start:start+len(numeric_cols)])

    grid = GridSearchCV(RandomForestClassifier(random_state=77), {'n_estimators':[100],'max_depth':[None],'min_samples_split':[2]}, scoring='roc_auc', cv=StratifiedKFold(5,shuffle=True,random_state=77), n_jobs=-1)
    clf = CalibratedClassifierCV(grid, cv=5, method='sigmoid')
    clf.fit(X_train, y_train)

    y_proba = clf.predict_proba(X_val)[:,1]
    thr = apply_dynamic_threshold(y_val, y_proba)
    ...
    disp.plot()
    plt.title(f"Matriz de Confusão - {checkpoint}")
    plt.show()

    joblib.dump((clf, scaler, thr, auc, acc), model_path)
    joblib.dump((model_w2v, champ_freq, numeric_cols, league_encoding, auc), meta_path)
    return clf, model_w2v, scaler, numeric_cols, league_encoding, auc, acc
...
if __name__ == '__main__':
    ...
    league_encoding_global = df.groupby("league")["result"].mean().to_dict()
    ...
    for cp in CHECKPOINTS:
        modelo = treinar_pipeline(df, cp, league_encoding_global, champ_freq, forcar_retreinamento=False)
        modelos[cp] = modelo
